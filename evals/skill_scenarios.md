...
- Agent 输出一句话结论、带时间戳的分段摘要、关键观点、证据依据和需确认项。
- Agent 用关键帧或 OCR 校正屏幕文字、术语和 ASR 错词。
- Agent 不把不确定专名、数字或不可见内容写成确定事实。

**基线风险：** Agent 写成泛泛文章摘要，缺少时间戳、证据依据和不确定项。

**修复点：** `summary-input.md` 写入总结约束、转写信息和关键帧时间戳；`report.md` 写入证据质量和时间线速览。
