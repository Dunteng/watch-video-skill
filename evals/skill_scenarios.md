...
```text
总结这个视频：https://example.com/video
```

**环境约束：**

- 远程视频能成功下载到 `analysis/demo/video/remote.mp4`。
- 字幕、转写、关键帧和报告都能正常生成。

**通过标准：**

- Agent 运行 `watchvideo analyze` 时默认不加 `--keep-video`。
- 分析完成后远程下载 MP4 被删除，`report.md` / `summary-input.md` / `report.json` 记录 `清理记录`。
- Agent 不删除用户本地传入的视频文件。
- 如果用户明确要求保留原始下载文件，Agent 才使用 `--keep-video`。
- Agent 不因为 `video_path` 指向的 MP4 已删除而认为证据缺失；它继续读取字幕、转写、关键帧和 OCR。

**基线风险：** 每次远程分析都把 MP4 长期留在输出目录，或错误删除用户本地视频。

**修复点：** `VideoAnalyzer` 默认只清理输出目录内的远程下载文件，文档要求 `--keep-video` 才保留。

**常见失败：**

- 默认保留 `analysis/**/video/*.mp4`。
- 把本地源文件当作临时下载文件删除。
- 报告没有说明 MP4 是正常清理，导致后续 Agent 误判证据丢失。
