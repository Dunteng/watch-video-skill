...
def render_summary_prompt(report: dict[str, Any], chunk_seconds: float = 300.0) -> str:
    ...
    lines = [
        ...
        "- 技术类、面试类、教程类或方法论类视频默认使用这个结构；其他类型视频可按内容裁剪不适用部分。",
        "- 每个关键结论尽量带时间戳或画面/OCR依据；缺少视频证据时保留标题并写“视频未明确提到”。",
        "- 转写噪声、专名、数字、代码或屏幕文字要用关键帧/OCR校正。",
        "",
        "## 技术/面试类默认报告结构",
        "",
        "### 一句话总结",
        "- 用一句话说清视频主张、适用场景和核心结论，不写空泛主题句。",
        "- 推荐句式：这个视频主张 `<观点>`，适用于 `<场景>`，核心结论是 `<结论>`。",
        "",
        "### 视频主线",
        "- 按时间线或论证链路写 3-6 个节点，每个节点尽量带时间戳。",
        "- 节点格式：`[时间戳] 问题/场景 -> 观点 -> 论据 -> 结论或转折`。",
        "",
        "### 核心概念拆解",
        "- 每个概念按 `概念 / 定义 / 边界 / 例子 / 为什么重要 / 视频证据` 拆解。",
        "- 不要只罗列名词；如果边界或例子没有证据，写“视频未明确提到”。",
        "",
        "### 常见误区",
        "- 每个误区按 `误区 / 为什么错 / 正确说法 / 证据` 写。",
        "- 只写视频明确反驳或提醒的误区；没有证据时写“视频未明确提到”。",
        "",
        "### 可执行方法",
        "- 把观点转成 `前提 / 步骤 / 检查清单 / 适用场景 / 不适用场景`。",
        "- 方法必须来自视频证据；不要把常识扩展成视频观点。",
        "",
        "### 面试回答模板",
        "- 给出 `30 秒版 / 2 分钟版 / 追问应对 / 不推荐回答`。",
        "- 回答要保留可替换变量、关键术语和证据边界；证据不足时写清限制。",
        "",
        "### 转写校正说明",
        "- 按 `可疑原文 / 建议校正 / 置信度 / 校正依据` 列出问题。",
        "- 覆盖 ASR 可能听错的术语、人名、数字、代码、命令和英文缩写。",
        "",
        "## 警告",
        "",
    ]
