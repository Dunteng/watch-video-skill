...
总结、分析、看懂或“讲了什么”类请求完成后，把最终内容写回 `report.md` 的 `视频内容总结` 区块；只有用户明确要求不要写文件时才跳过。**不要用标题、简介、搜索结果或同主题材料补内容。** 某一节缺少视频证据时，保留标题并写“视频未明确提到”。报告里不要逐张列出关键帧路径或 score；只保留关键帧数量和目录。
